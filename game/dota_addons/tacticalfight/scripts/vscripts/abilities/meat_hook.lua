...
LinkLuaModifier( "modifier_meat_hook_followthrough_lua", "lua_modifiers/modifier_meat_hook_followthrough_lua.lua" ,LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_meat_hook_lua", "lua_modifiers/modifier_meat_hook_lua.lua" ,LUA_MODIFIER_MOTION_HORIZONTAL )
...
function pudge_meat_hook_lua:OnProjectileHit( hTarget, vLocation )
	...
	if self.bRetracting == false then
		if hTarget ~= nil and ( not ( hTarget:IsCreep() ) ) then
		...
		if hTarget ~= nil then
			...
			if hTarget:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
				local damage = {
						...
						damage = self.hook_damage + (self.hook_distance / 50),
		...
		local flDistance = vVelocity:Length2D() - flPad - 720
		...
		if self.hVictim ~= nil then
			...
			if vVictimPosCheck:Length2D() > flPad + 720 then
