...
function powershot_initialize( keys )
	...
	local point = caster_location + forward_vector * ability.powershot_max_range

	-- Ability variables
	ability.powershot_damage_percent = 1.0
	...
	Timers:CreateTimer( function()
			...
			for k, v in pairs( ability.powershot_units_array ) do
				...
				if ability.powershot_units_hit[ k ] == false and powershot_distance( v:GetAbsOrigin(), ability.powershot_currentPos ) <= ability.powershot_radius then
					...
					print(ability:GetAbilityDamage() * ability.powershot_damage_percent)
					local damageTable =
					{
						victim = v,
						attacker = caster,
